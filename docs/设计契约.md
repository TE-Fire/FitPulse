...
| 方法 | 路径 | 鉴权 | 请求 | 响应 data |
|---|---|---|---|---|
| POST | `/register` | 否 | `{username, email?, password}` | null |
| POST | `/login` | 否 | `{username, password}` | `{accessToken, refreshToken, userId, username}` |
| POST | `/refresh` | 使用refresh | `{refreshToken}` | 同login |
| POST | `/logout` | 是 | — | null |
